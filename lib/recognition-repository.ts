...
export async function recognitionData(league:Pick<League,'players'|'matches'|'seasons'>){
 if(!env.DB)throw Error('Base de données indisponible');
 const stored=(await env.DB.prepare('SELECT season_id, data FROM recognition_seasons').all()).results as {season_id:string;data:string}[];
 ...
 for(const season of league.seasons.filter(s=>!s.demo)){
  ...
  if(finished){const frozen=awards.map(a=>({...a,status:'final' as const}));await env.DB.prepare('INSERT OR IGNORE INTO recognition_seasons(season_id,data,finalized_at) VALUES(?,?,?)').bind(season.id,JSON.stringify(frozen),new Date().toISOString()).run();const record=await env.DB.prepare('SELECT data FROM recognition_seasons WHERE season_id=?').bind(season.id).first<{data:string}>();results.push({seasonId:season.id,seasonName:season.name,finalized:true,awards:JSON.parse(record!.data)});}else results.push({seasonId:season.id,seasonName:season.name,finalized:false,awards});
 }
...
export async function exportRecognitionHistory(){if(!env.DB)throw Error('Base de données indisponible');const stored=(await env.DB.prepare('SELECT season_id, data, finalized_at FROM recognition_seasons').all()).results as {season_id:string;data:string;finalized_at:string}[];return stored.map(s=>({seasonId:s.season_id,finalizedAt:s.finalized_at,awards:JSON.parse(s.data)}))}
export async function restoreRecognitionHistory(history:import('./recognition-history').RecognitionHistory){if(!env.DB)throw Error('Base de données indisponible');if(!history.length)return;await env.DB.batch(history.map(s=>env.DB!.prepare('INSERT OR IGNORE INTO recognition_seasons(season_id,data,finalized_at) VALUES(?,?,?)').bind(s.seasonId,JSON.stringify(s.awards),s.finalizedAt)))}
